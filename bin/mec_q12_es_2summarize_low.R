...
###Load in Data file -------------------
d12<-read_csv("./results/q12_es_long_low.csv")%>%
  glimpse()
...
# add in 1 missing level -------------------
missing1<-c("biodiversity",
           "support a diversity of marine life",
           "Disagree", 0, 222, 0.0)
missing1<-data.frame(t(missing1))%>%
  glimpse()
row.names(missing1)<-"1"
colnames(missing1)<-c("services","services2","value","n_val","n_tot","pct")
missing2<-missing1%>%
  mutate(n_val=as.numeric(n_val),
         n_tot=as.numeric(n_tot),
         pct=as.numeric(pct))%>%
  glimpse()
glimpse(d12f1)

d12f<-rbind(d12f1,missing2)
